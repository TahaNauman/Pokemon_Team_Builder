...
def select_region():
    """Prompt the user to select a region and return the filtered DataFrame and region name."""
...
def select_starter(region):
    ...
    for s in starters[region]:
        print(f"  - {s}")

    while True:
        starter = input("\nChoose your starter: ").title()
        if starter not in starters[region]:
            print(f"Invalid starter. Choose from: {', '.join(starters[region])}")
        else:
            final_form = final_evolution[starter]
            print(f"\nYou chose {starter} → Final evolution: {final_form}")
            unchosen = [s for s in starters[region] if s != starter]
